...
if [[ "$(hyprctl eval 'return true' 2>&1)" == ok ]]; then
	...
	hyprctl eval "
if _G.portfolio_vm_rule then _G.portfolio_vm_rule:set_enabled(false) end
_G.portfolio_vm_rule = hl.window_rule({ name = 'portfolio-vm-window', match = { class = '^(qemu.*)$' }, workspace = '$workspace' })
